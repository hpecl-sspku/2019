...
if __name__ == '__main__':
    ...
    # ===================== 语音识别 =====================
    start = time.time()
    req = urllib.request.Request(ASR_URL, post_data.encode('utf-8'))
    #print(req)
   ...
   # print(result_str)
    ...
    client_id = 'stZjhWVjq1vLoIvL0YOlRvZL'
    # client_secret 为从UNIT的【发布上线】模块进入百度云创建应用后获取的Secret Key
    client_secret = 'yOwRiWwY9LOdL0BpPBXE2tGy7Uz6Ee4E'
    ...
    response = urllib.request.urlopen(request, context=gcontext2)
    ...
    if content:
        ...
        text = str(data['result']['response']['action_list'][0]['say'])
        synthesis.syn(text)
        print ('意图: ' + intent)
    # # ===================== 调用Falsk API接口 =====================
