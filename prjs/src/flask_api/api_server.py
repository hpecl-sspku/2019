...
def show_temp():
    ...
    cursor = c.execute("SELECT time, temperature, humidity, light, noise, co2_simulation from myhome_nodedata")
    #获取结果数组
    ...
    conn.commit()
    conn.close()
    return render_template('show_tmp.html', temp = data[-1][1], humi= data[-1][2],light=data[-1][3],noise=data[-1][4], co2=data[-1][5])
